...
class CheckoutController extends Controller
{
    ...
    public function checkout1Store(Request $request){
       ...
       [
        ...
        Session::put('checkout',[     //stored values in session & pass next
    ...
    public function checkoutStore(){
       ...
       $order['order_number']=Str::upper('ORD-'.Str::random(6)); //create random for order.no
      // return Session::get('checkout'); 
       ...
       $order['payment_method']=Session::get('checkout')['1']['payment_method'];  //get previous page values using session
       ...
       Mail::to($order['email'])->bcc($order['semail'])->cc('gihanamarasinghew@gmail.com')->send(new OrderMail($order));
             //dd('Mail is sent');
      ...
      // return $order;
       ...
       foreach(Cart::instance('shopping')->content() as $item){  //store multiple products in one id
          $product_id[]=$item->id;
