                        ...
                        <div class="widget price mb-30">
                            ...
                            <div class="widget-desc">
                                <div class="slider-range">
                                    ...
                                    <input type="text" id="amount"  readonly style="width:60%;border:0;" value="$@if(!empty($_GET['price']))  {{$price[0]}} @else {{Helper::minPrice()}} @endif - $@if(!empty($_GET['price']))  {{$price[1]}} @else {{Helper::maxPrice()}} @endif">
                                   <!-- <div class="range-price" id="amount">Price: {{Helper::minPrice()}} - {{Helper::maxPrice()}}</div>
                                   --> <button type="submit" class="btn btn-sm btn-primary float-right" style="margin: 12px 0px 12px opx;height:30px;line-height:22px;">Filter</button>
                                    </div>
                                </div>
                            </div>
                        </div>
                       ...
                       @if(count($brands)>0)
                        ...
                        <div class="widget size mb-30">
                            ...
                            <div class="widget-desc">
                             ...
                             <!-- Single Checkbox -->
                                <div class="custom-control custom-checkbox d-flex align-items-center mb-2">
                                    <input type="checkbox" id="customCheck11" class="form-control-input" @if(!empty($_GET['size']) && $_GET['size']=='S') checked @endif name="size" value="S" onchange="this.form.submit();">
                                    <label class="form-control-label" for="customCheck11"> Small<span class="text-muted">({{\App\Models\Product::where(['status'=>'active','size'=>"S"])->count()}})</span></label>
                                </div> 

                                <!-- Single Checkbox -->
                                <div class="custom-control custom-checkbox d-flex align-items-center mb-2">
                                    <input type="checkbox"  id="customCheck12" class="form-control-input" @if(!empty($_GET['size']) && $_GET['size']=='M') checked @endif name="size" value="M" onchange="this.form.submit();">
                                    <label class="form-control-label" for="customCheck12"> Medium<span class="text-muted">({{\App\Models\Product::where(['status'=>'active','size'=>"M"])->count()}})</span></label>
                                </div> 

                                <!-- Single Checkbox -->
                                <div class="custom-control custom-checkbox d-flex align-items-center mb-2">
                                    <input type="checkbox"  id="customCheck13" class="form-control-input" @if(!empty($_GET['size']) && $_GET['size']=='L') checked @endif name="size" value="L" onchange="this.form.submit();">
                                    <label class="form-control-label" for="customCheck13"> Large<span class="text-muted">({{\App\Models\Product::where(['status'=>'active','size'=>"L"])->count()}})</span></label>
                                </div> 

                                <!-- Single Checkbox -->
                                <div class="custom-control custom-checkbox d-flex align-items-center mb-2">
                                   <input type="checkbox"  id="customCheck14" class="form-control-input" @if(!empty($_GET['size']) && $_GET['size']=='XL') checked @endif name="size" value="XL" onchange="this.form.submit();">
                                    <label class="form-control-label" for="customCheck14"> Extra Large<span class="text-muted">({{\App\Models\Product::where(['status'=>'active','size'=>"XL"])->count()}})</span></label>
                                </div> 
                            </div>
                        </div>
                ...
                <div class="col-12 col-sm-7 col-md-8 col-lg-9">
                    ...
                    <div class="shop_top_sidebar_area d-flex flex-wrap align-items-center justify-content-between">
                       ...
                       <select id="sortBy" name="sortBy" onchange="this.form.submit();" class="small right">
                            ...
                            <option value="priceAsc" @if(!empty($_GET['sortBy']) && $_GET['sortBy']=='priceAsc') selected @endif >Price-Lower To High</option>
                            <option value="priceDesc"  @if(!empty($_GET['sortBy']) && $_GET['sortBy']=='priceDesc') selected @endif>Price Higher To Lower</option>
                            <option value="titleAsc" @if(!empty($_GET['sortBy']) && $_GET['sortBy']=='titleAsc') selected @endif>Alphabatical Ascending</option>
                            <option value="titleDesc" @if(!empty($_GET['sortBy']) && $_GET['sortBy']=='titleDesc') selected @endif>Alphabatical Decending</option>
                             <option value="discAsc" @if(!empty($_GET['sortBy']) && $_GET['sortBy']=='discAsc') selected @endif>Discount -Lower To High</option>
...
<script>
    $(document).ready(function (){
        if($('#slider-range').length>0){
            const max_value=parseInt($('#slider-range').data('max') || 500);  //set slider price range
